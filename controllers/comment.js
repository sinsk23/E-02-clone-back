...
class CommentController{
    ...
    getComment = async(req, res, next) =>{
     ...
     if(!finditemkey){
        return res.status(400).json({result:false, errormessage:'게시글이 존재하지 않습니다.'})   
     }
    ...
    editComment = async(req, res, next) =>{
        ...
        return res.status(201).json({result : true, message: '후기(댓글)을 수정했습니다.'});
            ...
            return res.status(401).json({ result : false, errormessage: "댓글 수정에 실패하셨습니다"});
          }
    }
    ...
    delComment = async(req, res, next) =>{
        ...
        return res.status(200).json({result : true, message:"후기(댓글)을 삭제했습니다."});
            ...
            return res.status(401).json({ result : false, errormessage: "댓글 삭제에 실패하셨습니다"});
          }
